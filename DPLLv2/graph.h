...
Status InsertVex(ALGraph *G, int key, int value)
{
    if (!G)
        return ERROR;
    ...
    {
        int prev = G->size;
        VNode **new = (VNode **)realloc(G->vert, sizeof(VNode *) * (G->size + SIZE_INCREAMENT));
        if (!new)
            exit(OVERFLOW);
        G->vert = new;
...
Status InsertArc(ALGraph *G, int vkey, int wkey, int info)
{
    if (!G)
        return ERROR;
